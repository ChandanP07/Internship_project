...
import { and, desc, eq, getTableColumns, inArray, sql, lt } from "drizzle-orm";

import { db } from "../db/index";
import { submissions, assignments, classes, enrollments, user } from "../db/schema/index";
  ...
  try {
    ...
    const rows = await db
      .select({
        ...
        assignment: { id: assignments.id, title: assignments.title, dueDate: assignments.dueDate },
      })
      .from(submissions)
...
// List submissions (for teachers/admins)
router.get("/", requireAuth(), async (req: any, res) => {
  try {
    const authUser = req.user;
    const { assignmentId, studentId, classId, page = 1, limit = 50 } = req.query;
    ...
    if (authUser.role === "teacher") {
      // Teachers can only see submissions for assignments in their classes
    ...
    const rows = await db
      .select({
        ...getTableColumns(submissions),
        assignment: { id: assignments.id, title: assignments.title, dueDate: assignments.dueDate, classId: assignments.classId },
        student: { id: user.id, name: user.name, email: user.email },
        reviewer: { id: user.id, name: user.name },
      })
      .from(submissions)
      .leftJoin(assignments, eq(submissions.assignmentId, assignments.id))
      .leftJoin(user, eq(submissions.studentId, user.id))
      .leftJoin(user, eq(submissions.reviewedBy, user.id))
      .where(filters.length > 0 ? and(...filters) : undefined)
      .orderBy(desc(submissions.submittedAt))
      .limit(limitPerPage)
      .offset(offset);
...
router.get("/:id", requireAuth(), async (req: any, res) => {
  try {
    ...
    if (authUser.role === "teacher") {
      ...
      const canManage = await canManageClass(authUser, record.assignment.classId);
      if (!canManage) {
        return res.status(403).json({ error: "Forbidden" });
      }
    }
...
router.post("/", requireAuth(["student"]), async (req: any, res) => {
  try {
    ...
    const [assignment] = await db
      .select({ id: assignments.id, classId: assignments.classId, dueDate: assignments.dueDate, status: assignments.status })
      .from(assignments)
    ...
    const canAccess = await canStudentAccessClass(authUser.id, assignment.classId);
    if (!canAccess) {
    ...
    const [existing] = await db
      ...
      .from(submissions)
      .where(and(eq(submissions.assignmentId, parsedAssignmentId), eq(submissions.studentId, authUser.id)));

    if (existing) {
      return res.status(409).json({ error: "Already submitted" });
    }

    // Determine status: late if submitted after due date
    const now = new Date();
...
router.patch("/:id", requireAuth(["teacher", "admin"]), async (req: any, res) => {
  try {
    ...
    // Get assignment to check class ownership
    const [assignment] = await db
...
// Delete submission (only by student if not graded)
  ...
  try {
    ...
    if (authUser.role === "student") {
      ...
      if (existing.status === "graded") {
        return res.status(403).json({ error: "Cannot delete graded submission" });
      }
    } else if (authUser.role === "teacher" || authUser.role === "admin") {
      // Get assignment to check class ownership
      const [assignment] = await db
        .select({ classId: assignments.classId })
        .from(assignments)
